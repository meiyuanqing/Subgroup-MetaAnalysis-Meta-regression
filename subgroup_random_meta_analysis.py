...
def subgroup_random_effect_meta_analysis(effect_size, variance, subgroup):
    ...
    subgroups = sorted(set(subgroup))
    # subgroups = sorted(set(subgroup.tolist()))
    print("the subgroups are ", subgroups)
    dic = {"effect_size": effect_size,
           "effect_size_variance": variance,
           "effect_size_subgroup": subgroup}
    ...
    for subgroup in subgroups:
        ...
        print("the current subgroup is ", subgroup)
        sum_Wi = 0
        sum_WiWi = 0
        sum_WiYi = 0  # Sum(Wi*Yi), where i ranges from 1 to k, and k is the number of studies
        sum_WiYiYi = 0  # Sum(Wi*Yi*Yi), where i ranges from 1 to k, and k is the number of studies
        ...
        d[subgroup + "_C"] = C
        d[subgroup + "_mean"] = randomMean
        d[subgroup + "_stdError"] = randomStdError
        d[subgroup + "_LL_CI"] = randomMean - 1.96 * randomStdError  # The 95% lower limits for the summary effect
        d[subgroup + "_UL_CI"] = randomMean + 1.96 * randomStdError  # The 95% upper limits for the summary effect
        # 20210719 adds the 84% CI for the summary effect
        d[subgroup + "_LL_CI_84"] = randomMean - 1.4051 * randomStdError  # The 84% lower limits for the summary effect
        d[subgroup + "_UL_CI_84"] = randomMean + 1.4051 * randomStdError  # The 84% upper limits for the summary effect
        # a Z-value to test the null hypothesis that the mean effect is zero;norm.cdf() 返回标准正态累积分布函数值
        d[subgroup + "_ZValue"] = randomMean / randomStdError
        d[subgroup + "_pValue_Z"] = 2 * (1 - norm.cdf(np.abs(randomMean / randomStdError)))
        # 20210414 双侧检验时需要增加绝对值符号np.abs
        d[subgroup + "_Q"] = Q
        d[subgroup + "_df"] = df
        d[subgroup + "_pValue_Q"] = pValue_Q
        d[subgroup + "_I2"] = I2
        d[subgroup + "_tau"] = T2 ** 0.5
        # tau、randomMean 已知情况下的新出现的study的effctsize所落的区间
        d[subgroup + "_LL_ndPred"] = randomMean - 1.96 * (T2 ** 0.5)
        d[subgroup + "_UL_ndPred"] = randomMean + 1.96 * (T2 ** 0.5)
        # tau、randomMean 未知情况（估计）下的新出现的study的effctsize所落的区间
        # stats.t.ppf(0.975,df)返回学生t分布单尾alpha=0.025区间点(双尾是alpha=0.05)的函数，它是stats.t.cdf()累积分布函数的逆函数
        d[subgroup + "_LL_tdPred"] = randomMean - stats.t.ppf(0.975, df) * (
                    (T2 + randomStdError * randomStdError) ** 0.5)
        # tau、randomMean 未知情况（估计）下的新出现的study的effctsize所落的区间
        d[subgroup + "_UL_tdPred"] = randomMean + stats.t.ppf(0.975, df) * (
                    (T2 + randomStdError * randomStdError) ** 0.5)
    ...
    if study_number == 1:
        ...
        pooled_T2 = (pooled_Q - pooled_df) / pooled_C  # sample estimate of tau squared

    if pooled_T2 < 0:
        pooled_T2 = 0  # 20210411，Set to 0 if T2 is less than 0.   M.Borenstein[2009] P114
    ...
    if study_number == 1:
        ...
        # the proportion of the observed variance reflects real differences in effect size
    if pooled_I2 < 0:
        pooled_I2 = 0  # 20210418，Set to 0 if I2 is less than 0.   M.Borenstein[2009] P110

    pooled_pValue_Q = 1.0 - stats.chi2.cdf(pooled_Q, pooled_df)  # pValue_Q = 1.0 - stats.chi2.cdf(chisquare, freedom_degree)
